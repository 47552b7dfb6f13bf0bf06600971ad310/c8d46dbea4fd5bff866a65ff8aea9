  ...
  <div class="mx-auto">
    ...
    <UCard class="mb-4">
      <UiFlex justify="center" class="max-w-[400px] w-full mx-auto ">
        ...
        >
          ...
          <UiText mini align="center" class="max-w-[99%]" weight="semibold">{{ useMoney().miniMoney(list3[1].level) }}</UiText>
        </UiFlex>
        ...
        >
          ...
          <UiText mini align="center" class="max-w-[99%]" weight="semibold">{{ useMoney().miniMoney(list3[0].level) }}</UiText>
        </UiFlex>
        ...
        >
          ...
          <UiText mini align="center" class="max-w-[99%]" weight="semibold">{{ useMoney().miniMoney(list3[2].level) }}</UiText>
        </UiFlex>
      </UiFlex>
    </UCard>
    ...
    <UiFlex type="col" class="grow gap-4" v-if="list7.length > 0">
      <UiFlex v-for="(item, index) in list7" :key="index" class="w-full gap-2">
        ...
        <UBadge color="gray" variant="soft" class="ml-auto px-3" size="md">Cấp {{ useMoney().toMoney(item.level) }}</UBadge>
      </UiFlex>
    </UiFlex>
  </div>
</template>
...
const submit = async () => {
  try {
    ...
    const data = await useAPI('game/public/rank/level', JSON.parse(JSON.stringify(state.value)))
